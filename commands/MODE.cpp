...
void Server::mode_command(std::string& cmd, int fd)
{
	...
	if(channelName[0] != '#' || !(channel = GetChannel(channelName.substr(1)))){
		...
		if(channel->getModeAtindex(2))
			keyy = channel->GetPassword();
		if(channel->getModeAtindex(4)){
			ss << channel->GetLimit();
			limitt = ss.str();
		}
		_sendResponse(RPL_CHANNELMODES(cli->GetNickName(), channel->GetName(), channel->getModes(),keyy,limitt) + \
...
bool Server::isvalid_limit(std::string& limit,Channel *channel)
{
	return (!(limit.find_first_not_of("0123456789")!= std::string::npos) && (std::atoi(limit.c_str()) > 0 && std::atoi(limit.c_str()) >= channel->GetClientsNumber()));
}
...
std::string Server::channel_limit(std::vector<std::string> tokens,  Channel *channel, size_t &pos, char opera, int fd, std::string chain, std::string& arguments)
{
	...
	if(opera == '+')
	{
		...
		{
			limit = tokens[pos++];
			if(!isvalid_limit(limit,channel))
			{
