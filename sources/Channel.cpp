...
std::string Channel::GetTopicSetter(){return this->topicSetter;}
...
Client* Channel::get_client_by_index(size_t index) {
...
bool Channel::change_adminToClient(std::string& nick){
	size_t i = 0;
	for(; i < admins.size(); i++){
		if(admins[i].GetNickName() == nick && admins.size() > 1 )
			break;
	}
	if(i < admins.size()){
